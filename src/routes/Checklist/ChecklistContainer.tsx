...
const FIXED_CHECKLIST_GROUPS: Array<{ id: string; title: string }> = [
  { id: 'opening', title: 'Mo cua cua hang' },
  { id: 'sales', title: 'Ban hang - Ban giao' },
  { id: 'cleaning', title: 'Ve sinh cua hang' },
  { id: 'inventory', title: 'Kiem tra hang hoa' },
  { id: 'closing', title: 'Chot ca - Cuoi ngay' },
];

const FIXED_GROUP_ORDER = new Map(FIXED_CHECKLIST_GROUPS.map((group, index) => [group.id, index]));
...
function buildFixedChecklistCategories(storeId: string, items: ChecklistItem[]): ChecklistCategory[] {
  return FIXED_CHECKLIST_GROUPS.map((group) => {
    const groupItems = items.filter((item) => item.categoryId === group.id);
    const countDone = groupItems.filter((item) => item.isCompleted).length;
    const countTotal = groupItems.length;
    ...
    return {
      ...
      countDone,
      countTotal,
      isCompleted: countTotal > 0 && countDone === countTotal,
    };
  });
}
...
function sortChecklistItems(items: ChecklistItem[]): ChecklistItem[] {
  return [...items].sort((a, b) => {
    const orderA = FIXED_GROUP_ORDER.get(a.categoryId) ?? Number.MAX_SAFE_INTEGER;
    const orderB = FIXED_GROUP_ORDER.get(b.categoryId) ?? Number.MAX_SAFE_INTEGER;
    if (orderA !== orderB) {
...
export default function ChecklistContainer({
  ...
  const [todayChecklistCategories, setTodayChecklistCategories] = useState<ChecklistCategory[]>(
    buildFixedChecklistCategories(DEFAULT_STORE_ID, []),
  );
  const [processChecklistCategories, setProcessChecklistCategories] = useState<ChecklistCategory[]>(
    buildFixedChecklistCategories(DEFAULT_STORE_ID, []).map((cat) => ({ ...cat, categoryType: 'process' })),
  );
  ...
  useEffect(() => {
    ...
    const loadChecklistByRole = async () => {
      ...
      try {
        ...
        if (todayCategoriesFromDb.length === 0) {
          todayCategoriesFromDb = await Promise.all(
            FIXED_CHECKLIST_GROUPS.map((group) =>
              checklistTodayCategoryService.create({
                storeId: activeStoreId,
                title: group.title,
                countDone: 0,
                countTotal: 0,
          ...
          todayCategoriesFromDb = todayCategoriesFromDb.map((cat) => ({ ...cat, categoryType: 'today' }));
        }

        if (processCategoriesFromDb.length === 0) {
          processCategoriesFromDb = await Promise.all(
            FIXED_CHECKLIST_GROUPS.map((group) =>
              checklistProcessCategoryService.create({
                storeId: activeStoreId,
                title: group.title,
                countDone: 0,
                countTotal: 0,
          ...
          processCategoriesFromDb = processCategoriesFromDb.map((cat) => ({ ...cat, categoryType: 'process' }));
        }
        ...
        const sortedDailyItems = sortChecklistItems(dailyItems);
  ...
  const handleToggleChecklistItem = async (itemId: string) => {
    ...
    const sortedItems = sortChecklistItems(updatedItems);
  ...
  const handleCreateRoleChecklist = async (
    ...
    try {
      ...
      const updatedItems = sortChecklistItems([...checklistItems, createdDailyItem]);
  ...
  const handleCreateRoleChecklistBatch = async (
    ...
    try {
      ...
      if (normalizedRoleCode === currentChecklistRoleCode) {
        ...
        const updatedItems = sortChecklistItems([...checklistItems, ...createdDailyItems]);
        setChecklistItems(updatedItems);
        recalculateChecklistProgress(updatedItems, todayChecklistCategories);
      }
